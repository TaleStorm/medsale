...
const Sidebar = () => {
  ...
  return (
    <div
      style={{ width: 246, gridTemplateRows: 'repeat(4, min-content) 1fr' }}
      className='h-full grid grid-flow-row auto-rows-max min-h-full bg-white'
    >
      <div className='bg-white' style={{ height: '4.5rem' }}>
        <div className='flex items-center p-4'>
          <img src='/icons/medx.png' />
          <p className='font-medium ml-2 text-warm-gray'>MedSale</p>
        </div>
      </div>
      <Divider />
      <div className='bg-white h-full pt-2'>
        {pages.map((item) => {
          ...
          if (isActive) setCurrentHeading(item.name);
          ...
          return (
            ...
            >
              <Link href={item.link} className='flex w-full h-full'>
